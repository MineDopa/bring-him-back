...
(function () {
  ...
  /* ================================================================
   模式管理（统一切换器核心）
   —— bookMode: 'single' | 'double' | null（null = 书没打开）
   —— 单页/双页两个引擎不再自己猜窗口宽度，一律看 bookMode
   —— 切换永远走 switchBook()：先清场 → 再按当前宽度启动对应引擎
   ================================================================ */
  let bookMode = null;
  ...
  /* ================================================================
   模块 5：渲染浮层
   —— v1.8：crew 气泡右侧 SuperChat 式逐条滑入，出现后逐个带退场动画滑出；
      旁白气泡左下角，出现后消失
   ================================================================ */
  const CHAT_LINGER = 6000; // crew 气泡停留时长
  const NARR_LINGER = 10000; // 旁白停留时长
  const BUBBLE_STEP = 2000; // 每条气泡间隔
  ...
  function initFlipClick() {
    ...
    flipbook.addEventListener('click', function (e) {
      // 只有双页模式生效（切换由 switchBook 统一决定）
      if (bookMode !== 'double') return;
  ...
  const singlePage = (function () {
    ...
    function next() {
      ...
      if (cur >= list.length - 1) {
        // 末页：合书（参考 booklet —— 封面往末页合并，两页绕左缘往中间合上）
        // 动画由 CSS keyframes 负责：flipped-back(0→180) + closing-cover(-180→0)
        busy = true;
        const last = list[cur];
        const first = list[0];
        last.classList.add('flipped-back'); // 末页往回翻
        first.classList.add('active', 'closing-cover'); // 封面盖回
        setTimeout(() => {
          first.classList.remove('closing-cover');
    ...
    function bindClick() {
      if (!flipbook) return;
      flipbook.addEventListener('click', function (e) {
        // 只有单页模式生效（切换由 switchBook 统一决定）
        if (bookMode !== 'single') return;
        const page = e.target.closest('.bookpage');
        if (!page) return;
  ...
  /* ================================================================
   模块 7：打开 / 关闭 + 统一切换器
   ================================================================ */
  // 清场：把两个引擎可能留下的所有痕迹抹掉（class + inline 样式 + 气泡）
  function resetBookState() {
    $$('.bookpage', flipbook).forEach((p) => {
      p.classList.remove('flipped', 'clickable', 'active', 'flipping', 'flipped-back');
      p.style.opacity = '';
      p.style.zIndex = '';
      p.style.pointerEvents = '';
    });
  ...
  // 双页引擎启动：设 z-index + 强制回第一对
  function startDouble() {
    initBook();          // z-index：外对高内对低
    currentPair = 0;     // 重置当前对
    showPair(0);         // 显示第一对（child1 左 + child2 右）
    renderSpread([0, 1]); // 渲染第一对的旁白+crew
  }

  // 统一切换器：先清场 → 按当前宽度启动对应引擎
  // force=true 时无视当前模式直接重启（用于打开书）
  function switchBook(force) {
    const want = window.innerWidth <= 720 ? 'single' : 'double';
    if (!force && want === bookMode) return; // 模式没变就不用动
    resetBookState(); // 先清掉上一套引擎的全部痕迹
    if (want === 'single') {
      singlePage.init(); // 单页：强制回第一页
    } else {
      startDouble(); // 双页：强制回第一对
    }
    bookMode = want;
  }
  ...
  function closeModal() {
    ...
    clearChat();
    bookMode = null; // 关闭后模式清空，下次打开重新决定
  }

  /* ================================================================
   模块 8：绑定触发
   ================================================================ */
  initFlipClick(); // 双页点击监听（内部看 bookMode 是否生效）
  singlePage.bindClick(); // 单页点击监听（内部看 bookMode 是否生效）
  ...
  // 书开着时窗口跨过 720px 边界 → 自动切换引擎（强制回第一页）
  window.addEventListener('resize', () => {
    if (modal && modal.classList.contains('is-open')) switchBook();
  });
